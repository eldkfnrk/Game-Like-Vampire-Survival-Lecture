...
public class AudioManager : MonoBehaviour
{
    ...
    AudioHighPassFilter bgmEffect;  // 특정 주파수보다 높은 오디오만 통과시키고 낮은 것은 잘라내는 오디오 이펙트 컴포넌트
    ...
    void SoundInit()
    {
        ...
        bgmEffect = Camera.main.GetComponent<AudioHighPassFilter>();  // AudioHighPassFilter 컴포넌트는 Listner 이펙트라서 Listner가 있는 메인 카메라에 붙여야 한다.(Listner가 있어야지만 이 컴포넌트를 추가할 수 있다.)
        ...
        for(int i = 0; i < sfxPlayers.Length; i++)
        {
            ...
            sfxPlayers[i].bypassListenerEffects = true;  // SFX가 AudioHighPassFilter와 같은 이펙트에 영향을 받으면 안 되기 때문에 영향을 받지 않도록 하는 요소를 true로 설정(AudioHighPassFilter는 리스너 이펙트라서 이 요소의 값을 변경하는 거고 다른 이펙트는 각자 알맞은 요소가 따로 있다.)ㅈ
    ...
    {
        // 인자 값으로 재생할지 멈출지를 받고 재생과 멈춤을 수행
        if (isPlay)
        {
            bgmPlayer.Play();
        }
    ...
    // 레벨 업을 할 때와 같이 게임 플레이가 멈추는 등의 동작을 할 때 bgm에 이펙트를 사용하도록 하는 함수
